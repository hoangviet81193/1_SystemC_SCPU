...
struct Memory: sc_module
{
  ...
  SC_CTOR(Memory)
  {
    ...
    SC_THREAD(flag_active);
    sc_event flag_1;
    SC_METHOD(flag_active);
    sensitive << flag_1;
  }
  ...
  void abc_xyz()
  {
    tlm::tlm_command cmd = socket.get_command();
    sc_dt::uint64    adr = socket.get_address() / 4;
    unsigned char*   ptr = socket.get_data_ptr();
    unsigned int     len = socket.get_data_length();
    unsigned char*   byt = socket.get_byte_enable_ptr();
    unsigned int     wid = socket.get_streaming_width();
    ...
    socket.set_response_status( tlm::TLM_OK_RESPONSE );
  }
